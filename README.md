...
联机不能重开，只给「回菜单」。认输 / 困毙 / 对手离开等非杀法终局走同一套，但没有母题与动作层，
标题用终局原因（认输不该演「绝杀」）。
...
npm run test:endgame     # 绝杀演出 136（含动作层：炮弹/马跃/车冲/将震颤）
...
npm run test:page        # 小游戏接线冒烟（含邀请回流、绝杀演出、账本、打击感）116
```
