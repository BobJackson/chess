...
function createLobbyScene(app) {
  ...
  scene.onEnter = function () {
    ...
    scene.buttons = [
      W.makeButton('create', pad, h * 0.30 + 76, bw, 46, '创建房间'),
      W.makeButton('join', pad, h * 0.30 + 76 + 60, bw, 46, '加入房间'),
      W.makeButton('back', pad, h * 0.30 + 76 + 120, bw, 46, '返回', 'ghost')
    ];
    scene.code = '';
    scene.myCode = '';
    scene.statusText = '';
  ...
  scene.refresh = function () {
    var s = scene.session;
    if (!s) return;
    if (s.state === 'waiting') scene.statusText = '房间号 ' + s.room + '，等待好友加入…';
